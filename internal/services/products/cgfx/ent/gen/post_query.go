...
type PostQuery struct {
	...
	withUserLikes      *UserLikeQuery
	modifiers          []func(*sql.Selector)
	loadTotal          []func(context.Context, []*Post) error
	withNamedAuthors   map[string]*UserQuery
	withNamedComments  map[string]*CommentQuery
	withNamedLikedBy   map[string]*UserQuery
	withNamedUserPosts map[string]*UserPostQuery
	withNamedUserLikes map[string]*UserLikeQuery
...
func (pq *PostQuery) QueryAuthors() *UserQuery {
...
// QueryUserPosts chains the current query on the "user_posts" edge.
func (pq *PostQuery) QueryUserPosts() *UserPostQuery {
	query := (&UserPostClient{config: pq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := pq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (pq *PostQuery) QueryUserLikes() *UserLikeQuery {
	query := (&UserLikeClient{config: pq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := pq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (pq *PostQuery) Clone() *PostQuery {
	...
	return &PostQuery{
		...
		withUserPosts: pq.withUserPosts.Clone(),
		withUserLikes: pq.withUserLikes.Clone(),
...
func (pq *PostQuery) WithAuthors(opts ...func(*UserQuery)) *PostQuery {
	query := (&UserClient{config: pq.config}).Query()
...
// the "user_posts" edge. The optional arguments are used to configure the query builder of the edge.
func (pq *PostQuery) WithUserPosts(opts ...func(*UserPostQuery)) *PostQuery {
	query := (&UserPostClient{config: pq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	pq.withUserPosts = query
	return pq
}
...
func (pq *PostQuery) WithUserLikes(opts ...func(*UserLikeQuery)) *PostQuery {
	query := (&UserLikeClient{config: pq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	pq.withUserLikes = query
	return pq
}
...
func (pq *PostQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Post, error) {
	var (
		...
		loadedTypes = [5]bool{
			...
			pq.withComments != nil,
			pq.withLikedBy != nil,
			pq.withUserPosts != nil,
			pq.withUserLikes != nil,
		}
	)
	...
	if query := pq.withAuthors; query != nil {
		if err := pq.loadAuthors(ctx, query, nodes,
			func(n *Post) { n.Edges.Authors = []*User{} },
			func(n *Post, e *User) { n.Edges.Authors = append(n.Edges.Authors, e) }); err != nil {
			return nil, err
		}
	}
	...
	if query := pq.withUserPosts; query != nil {
		if err := pq.loadUserPosts(ctx, query, nodes,
			func(n *Post) { n.Edges.UserPosts = []*UserPost{} },
			func(n *Post, e *UserPost) { n.Edges.UserPosts = append(n.Edges.UserPosts, e) }); err != nil {
			return nil, err
		}
	}
	if query := pq.withUserLikes; query != nil {
		if err := pq.loadUserLikes(ctx, query, nodes,
			func(n *Post) { n.Edges.UserLikes = []*UserLike{} },
			func(n *Post, e *UserLike) { n.Edges.UserLikes = append(n.Edges.UserLikes, e) }); err != nil {
			return nil, err
		}
	}
	for name, query := range pq.withNamedAuthors {
		if err := pq.loadAuthors(ctx, query, nodes,
			func(n *Post) { n.appendNamedAuthors(name) },
			func(n *Post, e *User) { n.appendNamedAuthors(name, e) }); err != nil {
			return nil, err
		}
	}
	...
	for name, query := range pq.withNamedLikedBy {
		if err := pq.loadLikedBy(ctx, query, nodes,
			func(n *Post) { n.appendNamedLikedBy(name) },
			func(n *Post, e *User) { n.appendNamedLikedBy(name, e) }); err != nil {
			return nil, err
		}
	}
	for name, query := range pq.withNamedUserPosts {
		if err := pq.loadUserPosts(ctx, query, nodes,
			func(n *Post) { n.appendNamedUserPosts(name) },
			func(n *Post, e *UserPost) { n.appendNamedUserPosts(name, e) }); err != nil {
			return nil, err
		}
	}
	for name, query := range pq.withNamedUserLikes {
		if err := pq.loadUserLikes(ctx, query, nodes,
			func(n *Post) { n.appendNamedUserLikes(name) },
			func(n *Post, e *UserLike) { n.appendNamedUserLikes(name, e) }); err != nil {
			return nil, err
		}
	}
...
func (pq *PostQuery) loadAuthors(ctx context.Context, query *UserQuery, nodes []*Post, init func(*Post), assign func(*Post, *User)) error {
	...
	query.Where(func(s *sql.Selector) {
		...
		s.Where(sql.InValues(joinT.C(post.AuthorsPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(post.AuthorsPrimaryKey[1]))
...
func (pq *PostQuery) loadUserPosts(ctx context.Context, query *UserPostQuery, nodes []*Post, init func(*Post), assign func(*Post, *UserPost)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Post)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(userpost.FieldPostID)
	}
	query.Where(predicate.UserPost(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(post.UserPostsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (pq *PostQuery) loadUserLikes(ctx context.Context, query *UserLikeQuery, nodes []*Post, init func(*Post), assign func(*Post, *UserLike)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Post)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(userlike.FieldPostID)
	}
	query.Where(predicate.UserLike(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(post.UserLikesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (pq *PostQuery) WithNamedAuthors(name string, opts ...func(*UserQuery)) *PostQuery {
	query := (&UserClient{config: pq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	if pq.withNamedAuthors == nil {
		pq.withNamedAuthors = make(map[string]*UserQuery)
	}
...
func (pq *PostQuery) WithNamedLikedBy(name string, opts ...func(*UserQuery)) *PostQuery {
	...
	if pq.withNamedLikedBy == nil {
		pq.withNamedLikedBy = make(map[string]*UserQuery)
	}
	pq.withNamedLikedBy[name] = query
	return pq
}

// WithNamedUserPosts tells the query-builder to eager-load the nodes that are connected to the "user_posts"
// edge with the given name. The optional arguments are used to configure the query builder of the edge.
func (pq *PostQuery) WithNamedUserPosts(name string, opts ...func(*UserPostQuery)) *PostQuery {
	query := (&UserPostClient{config: pq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	if pq.withNamedUserPosts == nil {
		pq.withNamedUserPosts = make(map[string]*UserPostQuery)
	}
	pq.withNamedUserPosts[name] = query
	return pq
}

// WithNamedUserLikes tells the query-builder to eager-load the nodes that are connected to the "user_likes"
// edge with the given name. The optional arguments are used to configure the query builder of the edge.
func (pq *PostQuery) WithNamedUserLikes(name string, opts ...func(*UserLikeQuery)) *PostQuery {
	query := (&UserLikeClient{config: pq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	if pq.withNamedUserLikes == nil {
		pq.withNamedUserLikes = make(map[string]*UserLikeQuery)
	}
	pq.withNamedUserLikes[name] = query
	return pq
}

...
func (cc *CommentCreate) check() error {
	...
	if _, ok := cc.mutation.CreatedAt(); !ok {
		return &ValidationError{Name: "created_at", err: errors.New(`gen: missing required field "Comment.created_at"`)}
	}
	if _, ok := cc.mutation.UpdatedAt(); !ok {
		return &ValidationError{Name: "updated_at", err: errors.New(`gen: missing required field "Comment.updated_at"`)}
	}
	if v, ok := cc.mutation.ID(); ok {
		if err := comment.IDValidator(v); err != nil {
			return &ValidationError{Name: "id", err: fmt.Errorf(`gen: validator failed for field "Comment.id": %w`, err)}
		}
	}
	if len(cc.mutation.AuthorIDs()) == 0 {
		return &ValidationError{Name: "author", err: errors.New(`gen: missing required edge "Comment.author"`)}
	}
	if len(cc.mutation.PostIDs()) == 0 {
		return &ValidationError{Name: "post", err: errors.New(`gen: missing required edge "Comment.post"`)}
	}
	return nil
}
